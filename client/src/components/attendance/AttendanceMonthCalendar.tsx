...
export function AttendanceMonthCalendar({
  ...
  const unmarkedWeekdays = useMemo(() => {
    return days.filter((iso) => iso <= today && isWeekdayIso(iso) && !markedDates.has(iso));
  }, [days, markedDates, today]);
  const dueWeekdays = useMemo(() => {
    return days.filter((iso) => iso <= today && isWeekdayIso(iso));
  }, [days, today]);

  const selectedMarked = markedDates.has(selectedDate);
  const selectedIsWeekday = /^\d{4}-\d{2}-\d{2}$/.test(selectedDate) && isWeekdayIso(selectedDate);
    ...
    >
      ...
      <div className="mt-1 grid grid-cols-7 gap-1">
        ...
        {days.map((iso) => {
          ...
          const unmarkedDue = !marked && weekday && !future;
          ...
          else if (!weekday || future) tone = "text-slate-400 hover:bg-white";
          ...
          return (
            <button
              ...
              aria-label={`${iso}${marked ? ", marked" : unmarkedDue ? ", not marked" : ""}`}
      ...
      <p className="mt-3 text-xs text-slate-600">
        ...
        <span className="inline-flex items-center gap-1">
          <span className="inline-block h-2.5 w-2.5 rounded-sm bg-amber-300" /> Not marked (weekdays)
        </span>
      </p>
      ...
      {!classId || !sectionId ? (
        ...
        <p className="mt-1 text-sm text-slate-700">
          {dueWeekdays.length === 0
            ? "No weekdays due yet in this month."
            : unmarkedWeekdays.length === 0
              ? "All weekdays in this month are marked (up to today)."
              : `${unmarkedWeekdays.length} weekday${unmarkedWeekdays.length === 1 ? "" : "s"} not marked yet this month.`}
        </p>
      ) : null}
